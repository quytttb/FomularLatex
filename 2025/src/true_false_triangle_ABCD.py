...
def generate_question(question_number):
    ...
    if correct_m_value is not None:
        ...
        for i in range(3):
            if i == 0:
                wrong_m = correct_m_value + Fraction(random.choice([-1, -2]), random.choice([1, 2]))
            elif i == 1:
                wrong_m = correct_m_value + Fraction(random.choice([1, 2]), random.choice([1, 2]))
            else:
                wrong_m = correct_m_value + Fraction(random.choice([-3, 3]), random.choice([1, 2]))
            wrong_m_answers.append(format_m_value(wrong_m))
    else:
        # For degenerate case, create arbitrary wrong answers
        wrong_m_answers = ["1", "-1", "0"]
    
    propositions.append({
        'question': f"Bốn điểm {l1}, {l2}, {l3}, D({x_d}; {y_d}; {z_expr}) đồng phẳng khi m",
        'correct_answer': f"\\( {correct_m_formatted} \\)",
