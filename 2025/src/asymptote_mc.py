...
def generate_wrong_answers(correct_slope, correct_intercept):
    ...
    wrong_answers = []
    correct_answer = (correct_slope, correct_intercept)
    ...
    while len(wrong_answers) < 3 and attempts < max_attempts:
        ...
        if len(wrong_answers) == 0:
            ...
            wrong_answer = (wrong_slope1, correct_intercept)
            ...
            wrong_answer = (correct_slope, wrong_intercept2)
        else:
            ...
            wrong_answer = (correct_slope, wrong_intercept3)
        
        # Only add if not duplicate and different from correct answer
        if wrong_answer not in wrong_answers and wrong_answer != correct_answer:
            wrong_answers.append(wrong_answer)
    
    # Ensure we have 3 different answers using fallback generation
    while len(wrong_answers) < 3:
        adjustment_slope = random.choice([Fraction(-2), Fraction(-1), Fraction(1), Fraction(2)])
        adjustment_intercept = random.choice([Fraction(-3), Fraction(-1), Fraction(1), Fraction(3)])
        fallback_slope = correct_slope + adjustment_slope
        fallback_intercept = correct_intercept + adjustment_intercept
        fallback_answer = (fallback_slope, fallback_intercept)
        
        if fallback_answer not in wrong_answers and fallback_answer != correct_answer:
            wrong_answers.append(fallback_answer)
