...
\(S_{ANB} = \dfrac{2}{3} \cdot ${HM} \cdot ${W} = ${S_ANB}\)

\(\dfrac{S_{EMF}}{S_{ANB}} = \left(\dfrac{KM}{HM}\right)^{3/2} = \left(\dfrac{${y_val}}{${HM}}\right)^{3/2} \Leftrightarrow S_{EMF} = ${S_EMF}\)

\(S_{IPQ} = \dfrac{2}{3} \cdot ${IN} \cdot ${w_bot} = ${S_IPQ}\)

\(\dfrac{S_{IEF}}{S_{IPQ}} = \left(\dfrac{KI}{NI}\right)^{3/2} = \left(\dfrac{${x_val}}{${IN}}\right)^{3/2} \Leftrightarrow S_{IEF} = ${S_IEF}\)

\(S_{\text{trắng}} = S_{ANB} + S_{IPQ} - (S_{IEF} + S_{EMF}) = ${S_ANB} + ${S_IPQ} - (${S_IEF} + ${S_EMF}) = ${S_white}\)

\(\Rightarrow S_{\text{cần tìm}} = S_{HCN} - S_{\text{trắng}} = ${S_rect} - ${S_white} = ${ans}\,(\text{m}^2)\)
