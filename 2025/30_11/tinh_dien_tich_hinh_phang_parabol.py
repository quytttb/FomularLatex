...
\(S_{MDQ} = \dfrac{2}{3} \cdot ${H} \cdot ${DA} = ${S_MDQ}\)

\(\dfrac{S_{EDF}}{S_{MDQ}} = \left(\dfrac{CD}{AD}\right)^{3/2} = \left(\dfrac{${x_val}}{${DA}}\right)^{3/2} \Leftrightarrow S_{EDF} = ${S_EDF}\)

\(S_{NBP} = \dfrac{2}{3} \cdot ${H} \cdot ${BG} = ${S_NBP}\)

\(\dfrac{S_{EBF}}{S_{NBP}} = \left(\dfrac{CB}{GB}\right)^{3/2} = \left(\dfrac{${y_val}}{${BG}}\right)^{3/2} \Leftrightarrow S_{EBF} = ${S_EBF}\)

\(\Rightarrow S_{\text{gạch chéo}} = S_{EDF} + S_{EBF} = ${S_EDF} + ${S_EBF} = ${ans}\,(\text{m}^2)\)
...
class ParabolCrossAreaQuestion:
    ...
    def calculate_answer(self) -> Tuple[str, Dict[str, Any]]:
        ...
        # Template uses "DA" and "BG".
        # Let's map: Code AD -> Template DA. Code GB -> Template BG.
        # Code BD -> Template BD.
        ...
        return ans_comma, {
            ...
            "DA": format_vn_number(AD),
            "BG": format_vn_number(GB),
            "BD": format_vn_number(BD), # Total distance between vertices
            "H_sq": format_vn_number(MQ_sq),
            "sum_DA_BG": format_vn_number(AD + GB),
            "x_val": format_vn_number(x_val, 3),
            "y_val": format_vn_number(y_val, 3),
            "EF_val": format_vn_number(EF_val, 3),
            "S_NBP": format_vn_number(S_NBP), # Right parabola area
            "S_MDQ": format_vn_number(S_MDQ), # Left parabola area
            "S_EBF": format_vn_number(S_EBF, 3),
            "S_EDF": format_vn_number(S_EDF, 3),
            "ans": ans_comma,
            "ans_dot": ans_dot,
            "diagram_q": diagram_q,
