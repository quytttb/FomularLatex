...
# Dạng vector: P = |a*MA + b*MB + c*MC|
TEMPLATE_Q = Template(
    r"""
Câu ${idx}. Cho biểu thức \(P = \left| ${coef_a}\overrightarrow{MA}${sign_b}${coef_b}\overrightarrow{MB}${sign_c}${coef_c}\overrightarrow{MC} \right|\) với \(A${coord_A}\), \(B${coord_B}\), \(C${coord_C}\).
...
Gọi \(G\) là điểm sao cho \(${coef_a}\overrightarrow{GA}${sign_b}${coef_b}\overrightarrow{GB}${sign_c}${coef_c}\overrightarrow{GC} = \overrightarrow{0}\).

Ta có: \(${coef_a}\overrightarrow{MA}${sign_b}${coef_b}\overrightarrow{MB}${sign_c}${coef_c}\overrightarrow{MC} = ${sum_coef}\overrightarrow{MG}\)

Suy ra: \(P = \left| ${sum_coef}\overrightarrow{MG} \right| = ${abs_sum_coef} \cdot MG\)

Tọa độ \(G = \dfrac{${coef_a_full} \cdot A${sign_b}${coef_b_full} \cdot B${sign_c}${coef_c_full} \cdot C}{${sum_coef}} = ${coord_G}\)
...
\(T = P_{min} = ${abs_sum_coef} \cdot d(G, ${plane_symbol}) = ${abs_sum_coef} \cdot ${distance_val} = ${pmin_val}\)
...
\(T = P_{min} = ${abs_sum_coef} \cdot d(G, (\alpha)) = ${abs_sum_coef} \cdot ${distance_val} = ${pmin_val}\)
...
class TimDiemMMinVectorQuestion:
    def __init__(self):
        self.A = [0, 0, 0]
        self.B = [0, 0, 0]
        self.C = [0, 0, 0]
...
def main():
    ...
    for i in range(num_questions):
        q = TimDiemMMinVectorQuestion()
        questions.append(q.generate_question(i+1, plane_option))
        
    content = "\n\\newpage\n".join(questions)
    latex = create_latex_document(content)
    
    output_path = os.path.join(os.path.dirname(__file__), "tim_diem_M_min_vector.tex")
    with open(output_path, "w", encoding="utf-8") as f:
        f.write(latex)
    print(f"Đã tạo {num_questions} câu hỏi trong tim_diem_M_min_vector.tex")
