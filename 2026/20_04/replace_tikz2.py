...
start_marker = r"\begin{tikzpicture}[line join = round, line cap=round,>=stealth,font=\footnotesize,scale=.8]"
end_marker = r"\end{tikzpicture}"

start_idx = c.find(start_marker)
end_idx = c.find(end_marker, start_idx) + len(end_marker)

if start_idx != -1 and end_idx != -1:
    c = c[:start_idx] + new_tikz + c[end_idx:]
