...
with open("21_03/parabola_arch_glass_questions.py", "r", encoding="utf-8") as f:
    content = f.read()
...
- Đáp án: làm tròn tổng chi phí đến một chữ số thập phân (nửa lên).
- Số lớp tối đa là 10 để hình TikZ không bị quá bé khi scale.

Trình bày LaTeX: dấu phẩy thập phân kiểu Việt Nam; không dùng \textbf{itemize} trong
lời giải (theo chuẩn đã dùng cho swimming_pool_area_questions.py).
'''
...
N_LAYERS_VALUES = list(range(4, 11))  # 4 to 10 layers
W_BASE_VALUES = list(range(4, 25))
...
TIKZ_LATEX_MAX_WIDTH_CMD = r"\ParabolaArchTikzMaxW"
TIKZ_LATEX_MAX_HEIGHT_QUESTION_CMD = r"\ParabolaArchTikzQMaxH"
TIKZ_LATEX_MAX_HEIGHT_SOLUTION_CMD = r"\ParabolaArchTikzSMaxH"

def _wrap_tikz_for_pdf(inner: str, max_height_cmd: str) -> str:
    r'''Bọc tikzpicture trong adjustbox để không tràn \linewidth và không quá cao.'''
    return (
        "\\begin{adjustbox}{max width="
        + TIKZ_LATEX_MAX_WIDTH_CMD
        + ",max height="
        + max_height_cmd
...
def generate_tikz_question(H, W, W_WALK, H_WALK, N_LAYERS, N_WALK_LAYERS, DY, DY_STR):
    A = 4 * H / (W * W)
    ...
    icon_scale = min(0.7, W_WALK / 2.5, H_WALK / 4.0)
    icon_x = -0.8 * icon_scale
    
    tikz = r'''\begin{tikzpicture}[>=stealth, scale=1.2,xscale=1, font=\footnotesize]
   % Parabol
   \draw[cyan!20, opacity=0.4, domain=-<W_HALF>:<W_HALF>,very thick, blue!70!black, samples=100] 
   plot (\x, {-<A>*\x*\x + <H>});
   
   % Các lớp ngang + nhãn
'''
    for i in range(1, N_WALK_LAYERS + 1):
        tikz += f'''   \\pgfmathsetmacro{{\\y}}{{{i}*{DY}}}
   \\pgfmathsetmacro{{\\x}}{{sqrt((<H>-\\y)/<A>)}}
   \\draw[thick, cyan!50!blue] (-\\x,\\y) -- (\\x,\\y);
   \\node[left,scale=0.6] at (-\\x+0.45,\\y-0.25*{DY}/0.5) {{Lớp {i}}};\n'''
        
    for i in range(N_WALK_LAYERS + 1, N_LAYERS + 1):
        tikz += f'''   \\pgfmathsetmacro{{\\y}}{{{i}*{DY}}}
   \\pgfmathsetmacro{{\\x}}{{sqrt((<H>-\\y)/<A>)}}
   \\draw[thick, cyan!50!blue] (-\\x,\\y) -- (\\x,\\y);
   \\node[left,scale=0.7] at (0.25,\\y-0.25*{DY}/0.5) {{Lớp {i}}};\n'''
        
    tikz += r'''   
   % Cửa
   \fill[orange!80] (-<W_WALK_HALF>,0) rectangle (<W_WALK_HALF>,<H_WALK>);
   \draw[very thick, brown!80!black] (-<W_WALK_HALF>,0) rectangle (<W_WALK_HALF>,<H_WALK>);
   
   % Đáy
   \draw[thick] (-<W_HALF>,0) -- (<W_HALF>,0);
...
\draw[fill=black] (0.37,0.23) .. controls +(-60.71:0.08) and +(104.82:0.09) .. (0.56,0.01) .. controls +(-78.4:0.03) and +(-7.1:0.07) .. (0.43,0) .. controls +(-174.06:0.06) and +(-36.62:0.09) .. (0.25,0.05) .. controls +(133.77:0.05) and +(-106.6:0.04) .. (0.23,0.16) .. controls +(172.87:0.04) and +(-47.56:0.04) .. (0.12,0.17) .. controls +(161.57:0.04) and +(-72:0.03) .. (0.06,0.25) .. controls +(45.45:0.13) and +(-139.82:0.13) .. (0.27,0.52) .. controls +(106.35:0.05) and +(-110.94:0.05) .. (0.29,0.66) .. controls +(43.15:0.14) and +(-120.39:0.16) .. (0.52,0.98) .. controls +(105.69:0.1) and +(-79.19:0.11) .. (0.51,1.29) .. controls +(90.68:0.11) and +(-88.95:0.1) .. (0.56,1.61) .. controls +(80.32:0.05) and +(-90.87:0.07) .. (0.54,1.78) .. controls +(-118.25:0.08) and +(43.23:0.07) .. (0.42,1.59) .. controls +(-100.12:0.09) and +(82.21:0.04) .. (0.38,1.38) .. controls +(-109.02:0.06) and +(-82.33:0.13) .. (0.23,1.39) .. controls +(87.77:0.08) and +(-106.95:0.08) .. (0.28,1.62) .. controls +(66.31:0.11) and +(-95.39:0.1) .. (0.36,1.9) .. controls +(74.4:0.13) and +(-100.48:0.11) .. (0.48,2.19) .. controls +(69.58:0.09) and +(-122.32:0.07) .. (0.63,2.34) .. controls +(101.22:0.04) and +(-93.63:0.05) .. (0.63,2.45) .. controls +(160.29:0.01) and +(-65.09:0.04) .. (0.57,2.52) .. controls +(139.42:0.03) and +(-90.41:0.08) .. (0.53,2.74) .. controls +(76.93:0.06) and +(-148.46:0.05) .. (0.65,2.88) .. controls +(15.5:0.1) and +(174.72:0.07) .. (0.88,2.9) .. controls +(-19.58:0.08) and +(106.35:0.04) .. (0.99,2.79) .. controls +(-101.61:0.05) and +(144.18:0.05) .. (0.99,2.66) .. controls +(-111.42:0.03) and +(52.53:0.05) .. (0.96,2.53) .. controls +(-155.31:0.06) and +(43.5:0.06) .. (0.82,2.44) .. controls +(-92.84:0.03) and +(87.23:0.03) .. (0.82,2.34) .. controls +(-40.52:0.2) and +(85.62:0.15) .. (0.97,1.94) .. controls +(-20.93:0.14) and +(132.63:0.18) .. (1.43,1.55) .. controls +(-90:0.06) and +(56.49:0.04) .. (1.4,1.41) .. controls +(178.14:0.04) and +(-1.93:0.04) .. (1.27,1.41) .. controls +(121.93:0.04) and +(-57.98:0.04) .. (1.2,1.52) .. controls +(103.92:0.04) and +(-38.57:0.05) .. (1.09,1.63) .. controls +(175.16:0.03) and +(-4.76:0.04) .. (0.99,1.7) .. controls +(-169.9:0.05) and +(100.73:0.04) .. (0.9,1.59) .. controls +(-64:0.12) and +(74.72:0.09) .. (0.95,1.31) .. controls +(-42.11:0.15) and +(98.72:0.14) .. (1.22,0.93) .. controls +(-84.72:0.06) and +(103.38:0.06) .. (1.25,0.76) .. controls +(-63.41:0.11) and +(91.99:0.08) .. (1.31,0.51) .. controls +(-53.82:0.11) and +(102.98:0.09) .. (1.4,0.26) .. controls +(-36.79:0.04) and +(143.16:0.04) .. (1.51,0.18) .. controls +(0:0.05) and +(180:0.06) .. (1.67,0.18) .. controls +(-47.53:0.07) and +(-0.53:0.07) .. (1.64,0.02) .. controls +(-165.74:0.06) and +(-22.76:0.07) .. (1.46,0.04) .. controls +(-148.94:0.06) and +(-4.77:0.07) .. (1.3,0) .. controls +(137.46:0.08) and +(-69.09:0.07) .. (1.24,0.18) .. controls +(125.81:0.1) and +(-91.19:0.09) .. (1.14,0.42) .. controls +(95.65:0.07) and +(-85.24:0.09) .. (1.06,0.68) .. controls +(108.14:0.12) and +(-55.9:0.12) .. (0.9,0.98) .. controls +(165.73:0.07) and +(66.1:0.06) .. (0.73,0.93) .. controls +(-53.66:0.07) and +(78.79:0.05) .. (0.75,0.77) .. controls +(-140.06:0.12) and +(38.48:0.11) .. (0.53,0.52) .. controls +(-116.62:0.05) and +(63.43:0.05) .. (0.46,0.39) .. controls +(-135.16:0.08) and +(82.23:0.06) .. (0.37,0.23);
   ...
   \foreach \x in {0,<DY>,...,<H_MINUS_DY>}
   \draw[<->] (<W_HALF_PLUS_05>,\x)--(<W_HALF_PLUS_05>,\x+<DY>)node[pos=0.5,right]{$<DY_STR>$ m};
   
   % Kích thước chiều cao
   \draw[<->] (-<W_HALF_PLUS_05>,0) -- (-<W_HALF_PLUS_05>,<H>);
   \node[rotate=90,above] at (-<W_HALF_PLUS_05>,<H_HALF>) {$<H>$ m};
   
   % Kích thước chiều rộng
   \draw[<->] (-<W_HALF>, -0.5) -- (<W_HALF>, -0.5);
   \node[below] at (0,-0.5) {$<W>$ m};
   
   % Kích thước cửa
   \draw[<->] (-<W_WALK_HALF>, -1) -- (<W_WALK_HALF>, -1);
   \node[below] at (0,-1) {$<W_WALK_STR>$ m};
   \path (0,<H_PLUS_05>) node[above]{\textbf{Khu Vực Ốp Lát Kính}};
   
   \path (<ICON_X>,0) pic[scale=<ICON_SCALE>]{Icon-Tau};
\end{tikzpicture}'''
    ...
    mapping = {
        ...
        "A": A,
        "H": H,
        "W_WALK_HALF": W_WALK / 2,
        "H_WALK": H_WALK,
        "H_MINUS_DY": H - DY,
        "W_HALF_PLUS_05": W / 2 + 0.5,
        "H_HALF": format_decimal_vn(H / 2, 1) if H/2 != int(H/2) else str(int(H/2)),
        "W": W,
        "W_WALK_STR": W_WALK_STR,
        "H_PLUS_05": H + 0.5,
        "ICON_X": icon_x,
    ...
    for k, v in mapping.items():
        tikz = tikz.replace(f"<{k}>", str(v))
        
    return _wrap_tikz_for_pdf(tikz, TIKZ_LATEX_MAX_HEIGHT_QUESTION_CMD)

def generate_tikz_solution(H, W, W_WALK, H_WALK, K, N_LAYERS, N_WALK_LAYERS, DY):
    A = 4 * H / (W * W)
    
    W_WALK_STR = format_decimal_vn(W_WALK, 1) if W_WALK != int(W_WALK) else str(int(W_WALK))
    H_WALK_STR = format_decimal_vn(H_WALK, 1) if H_WALK != int(H_WALK) else str(int(H_WALK))
    
    tikz = r'''\begin{tikzpicture}[>=stealth, scale=1.5,xscale=1, font=\footnotesize]
    % Parabol
    \draw[cyan!20, opacity=0.4, domain=-<W_HALF>:<W_HALF>,very thick, blue!70!black, samples=100] 
    plot (\x, {-<A>*\x*\x + <H>});
...
'''
    ...
    \node[below] at (0,-1) {$<W_WALK_STR>$ m};
    \path (0,<H_PLUS_05>) node[above]{\textbf{Khu Vực Ốp Lát Kính}};
    \draw[<->] (-<W_HALF>,-0.25)--(-<W_HALF_PLUS_05>,-0.25) node[below]{$0{,}5$ m};
    
    \draw[<-] (<LABEL_X_START>, <LABEL_Y>) -- (<LABEL_X_END>, <LABEL_Y>) node[right] {$x = d(y) = \sqrt{<K>(<H>-y)}$};
    
    % Trục mờ
    \draw[->, gray!60] (-<AXIS_X>,0) -- (<AXIS_X>,0);
    \draw[->, gray!60] (0,-0.5) -- (0,<AXIS_Y>);
\end{tikzpicture}'''

    label_y = H_WALK / 2 + DY / 2
    label_x_start = math.sqrt((H - label_y) / A)
    label_x_end = label_x_start + 1.0
    ...
    mapping = {
        ...
        "K": K,
        "LABEL_Y": label_y,
        "LABEL_X_START": label_x_start,
        "LABEL_X_END": label_x_end,
        "AXIS_X": W / 2 + 1.5,
        "AXIS_Y": H + 1.8
    }
    ...
    return _wrap_tikz_for_pdf(tikz, TIKZ_LATEX_MAX_HEIGHT_SOLUTION_CMD)
...
\begin{itemize}
    \item[$\diamond$] Lớp dưới cùng của cổng là $<C0_STR>$ triệu đồng/m$^2$.
    \item[$\diamond$] Càng lên cao, giá thi công mỗi lớp tiếp theo tăng thêm $<DC_STR>$ triệu đồng/m$^2$ so với lớp ngay dưới nó.
\end{itemize}
...
class ParabolaArchGlassQuestion:
    ...
    def calculate_values(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "tikz_question": generate_tikz_question(H, W, W_WALK, H_WALK, N_LAYERS, N_WALK_LAYERS, DY, dy_str),
            "tikz_solution": generate_tikz_solution(H, W, W_WALK, H_WALK, K, N_LAYERS, N_WALK_LAYERS, DY),
        }
    ...
    def generate_question_only(self, question_number: int = 1) -> Tuple[str, str]:
        ...
        answer = self.calculated_values["answer_tex"]
...
% Giới hạn kích thước hình TikZ đề / lời giải (không tràn khổ, tiết kiệm chiều dọc)
\newlength{\ParabolaArchTikzMaxW}
\newlength{\ParabolaArchTikzQMaxH}
\newlength{\ParabolaArchTikzSMaxH}
\setlength{\ParabolaArchTikzMaxW}{0.88\linewidth}
\setlength{\ParabolaArchTikzQMaxH}{0.28\textheight}
\setlength{\ParabolaArchTikzSMaxH}{0.25\textheight}
...
with open("21_03/parabola_arch_glass_questions.py", "w", encoding="utf-8") as f:
    f.write(new_code)
