...
def generate_question(question_number):
    """Generate an ABCD multiple choice question about triangles."""
    ...
    # Generate 4 propositions (a, b, c, d) but only choose one randomly
    propositions = []
    ...
    # Generate 3 wrong answers for foot coordinates
    wrong_foot_answers = []
    for i in range(3):
        # Modify different coordinates for each wrong answer
        if i == 0:
            wrong_x = foot_x_frac + Fraction(random.choice([-2, -1, 1, 2]))
            wrong_foot_answers.append(f"({format_coord_frac(wrong_x)}, {format_coord_frac(foot_y_frac)}, {format_coord_frac(foot_z_frac)})")
        elif i == 1:
            wrong_y = foot_y_frac + Fraction(random.choice([-2, -1, 1, 2]))
            wrong_foot_answers.append(f"({format_coord_frac(foot_x_frac)}, {format_coord_frac(wrong_y)}, {format_coord_frac(foot_z_frac)})")
        else:
            wrong_z = foot_z_frac + Fraction(random.choice([-2, -1, 1, 2]))
            wrong_foot_answers.append(f"({format_coord_frac(foot_x_frac)}, {format_coord_frac(foot_y_frac)}, {format_coord_frac(wrong_z)})")
    
    propositions.append({
        'question': f"{prop_text}",
        'correct_answer': f"\\( D{true_foot} \\)",
        'wrong_answers': [f"\\( D{ans} \\)" for ans in wrong_foot_answers],
    ...
    # Create 3 wrong altitudes by modifying coefficients
    wrong_altitude_answers = []
    for i in range(3):
        if i == 0:
            wrong_coeff = altitude_exact[0] + random.choice([-2, -1, 1, 2])
            wrong_altitude_answers.append(format_exact_fraction(wrong_coeff, altitude_exact[1], altitude_exact[2], altitude_exact[3]))
        elif i == 1:
            wrong_denom = altitude_exact[2] + random.choice([-1, 1])
            if wrong_denom != 0:
                wrong_altitude_answers.append(format_exact_fraction(altitude_exact[0], altitude_exact[1], wrong_denom, altitude_exact[3]))
            else:
                wrong_altitude_answers.append(format_exact_fraction(altitude_exact[0] + 1, altitude_exact[1], altitude_exact[2], altitude_exact[3]))
        else:
            # Change radicand
            wrong_rad = altitude_exact[1] + random.choice([1, 2, 3])
            wrong_altitude_answers.append(format_exact_fraction(altitude_exact[0], wrong_rad, altitude_exact[2], altitude_exact[3]))
    
    propositions.append({
        'question': f"{prop_text}",
        'correct_answer': f"\\( {true_altitude_str} \\)",
        'wrong_answers': [f"\\( {ans} \\)" for ans in wrong_altitude_answers],
    ...
    # Generate 3 wrong angles
    wrong_angle_answers = []
    for i in range(3):
        if i == 0:
            wrong_angle_answers.append(f"{angle + random.choice([-10, -5]):.1f}°")
        elif i == 1:
            wrong_angle_answers.append(f"{angle + random.choice([5, 10]):.1f}°")
        else:
            wrong_angle_answers.append(f"{angle + random.choice([-15, 15]):.1f}°")
    
    propositions.append({
        'question': f"{prop_text}",
        'correct_answer': f"\\( {true_angle} \\)",
        'wrong_answers': [f"\\( {ans} \\)" for ans in wrong_angle_answers],
    ...
    attempt_count = 0
    while coeff_m == 0 and attempt_count < 5:
    ...
    if coeff_m != 0:
        m_value = -const_term / coeff_m
        m_value = round(m_value, 1)  # Round to 1 decimal place for nice numbers
    else:
        # Fallback: set a simple case
        a_coeff, b_const, x_d, y_d = 1, 0, 0, 0
        m_value = 1.0
    ...
    m_value_formatted = format_m_value(m_value)
    
    # Generate 3 wrong m values
    wrong_m_answers = []
    for i in range(3):
        if i == 0:
            wrong_m = round(m_value + random.choice([-1, -0.5]), 1)
        elif i == 1:
            wrong_m = round(m_value + random.choice([0.5, 1]), 1)
        else:
            wrong_m = round(m_value + random.choice([-2, 2]), 1)
        wrong_m_answers.append(format_m_value(wrong_m))
    
    propositions.append({
        'question': f"Bốn điểm {l1}, {l2}, {l3}, D({x_d}; {y_d}; {z_expr}) đồng phẳng khi m",
        'correct_answer': f"\\( {m_value_formatted} \\)",
        'wrong_answers': [f"\\( {ans} \\)" for ans in wrong_m_answers],
        'x_d': x_d,
        'y_d': y_d,
        'a_coeff': a_coeff,
        'b_const': b_const,
    ...
    # Randomly choose one proposition to use for this question
    chosen_index = random.randint(0, 3)
    chosen_prop = propositions[chosen_index]
    ...
    question_stem = (
        ...
        f"{chosen_prop['question']}"
    )
    
    # Create all options (1 correct + 3 wrong) and shuffle them
    all_options = [chosen_prop['correct_answer']] + chosen_prop['wrong_answers']
    random.shuffle(all_options)
    correct_index = all_options.index(chosen_prop['correct_answer'])
    
    # Format question with ABCD options
    question_content = f"Câu {question_number}: {question_stem}\n\n"
    
    option_labels = ['A', 'B', 'C', 'D']
    for i, (label, option) in enumerate(zip(option_labels, all_options)):
        marker = '*' if i == correct_index else ''
        question_content += f"{marker}{label}. {option}\n\n"
    
    # Add solution based on the chosen proposition type
    question_content += "Lời giải:\n\n"
    
    if chosen_prop['type'] == 'foot_bisector':
        # For foot of angle bisector
        foot_data = chosen_prop
        from_vertex = l1
        vertex_coord = A
        ...
        question_content += foot_formula_explanation
        
    elif chosen_prop['type'] == 'altitude':
        # For altitude length
        alt_data = chosen_prop
        ...
        question_content += altitude_formula_explanation
        
    elif chosen_prop['type'] == 'angle':
        # For angle calculation
        angle_data = chosen_prop
        ...
        question_content += angle_formula_explanation
        
    else:  # chosen_prop['type'] == 'coplanar'
        # For coplanar condition
        coplanar_data = chosen_prop
        ...
        if coplanar_data['a_coeff'] == 1:
            if z_simplified_const == 0:
                term3_expansion = f"{cross_product[2]} \\cdot m"
            elif z_simplified_const > 0:
                term3_expansion = f"{cross_product[2]} \\cdot m + {cross_product[2]} \\cdot {z_simplified_const}"
            else:
                term3_expansion = f"{cross_product[2]} \\cdot m - {cross_product[2]} \\cdot {abs(z_simplified_const)}"
        elif coplanar_data['a_coeff'] == -1:
            if z_simplified_const == 0:
                term3_expansion = f"{cross_product[2]} \\cdot (-m)"
            elif z_simplified_const > 0:
            ...
            if z_simplified_const == 0:
                term3_expansion = f"{cross_product[2]} \\cdot ({coplanar_data['a_coeff']}m)"
            elif z_simplified_const > 0:
                term3_expansion = f"{cross_product[2]} \\cdot ({coplanar_data['a_coeff']}m) + {cross_product[2]} \\cdot {z_simplified_const}"
            else:
                term3_expansion = f"{cross_product[2]} \\cdot ({coplanar_data['a_coeff']}m) - {cross_product[2]} \\cdot {abs(z_simplified_const)}"
        ...
        # Update m_value to match the corrected calculation
        if coeff_of_m != 0:
            corrected_m_value = -total_constant / coeff_of_m
            coplanar_data['m_value'] = corrected_m_value
        ...
        if coeff_of_m == 0:
            ...
            # Format m_value as fraction using the corrected value
            m_value = coplanar_data['m_value']
            if isinstance(m_value, float):
                # Convert decimal to fraction
                from fractions import Fraction
                m_fraction = Fraction(m_value).limit_denominator()
        ...
        # Format the equation steps with proper signs
        ...
        # Format coeff_of_m with proper sign handling
        if coeff_of_m >= 0:
        ...
        question_content += det_explanation
    
    return question_content

def create_latex_document(questions, title="Câu hỏi Trắc nghiệm về Tam Giác"):
...
def main():
    ...
    try:
        ...
        # Write to file
        output_filename = "true_false_triangle_ABCD_questions.tex"
